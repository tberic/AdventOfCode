...
def calculate_outside_perimeter(xx, yy, region):
    ...
    while True:
        ...
        while grid[xx + DIRS[dir][0]][yy + DIRS[dir][1]] == region and grid[xx+DIRS_OUT[dir][0]][yy+DIRS_OUT[dir][1]] != region:
            visited[xx][yy] |= PERIMETER_OUT_MASKS[dir]
            xx += DIRS[dir][0]
            yy += DIRS[dir][1]
        ...
        if grid[xx+DIRS_OUT[dir][0]][yy+DIRS_OUT[dir][1]] == region:
            dir = (dir + 3) % 4
            xx += DIRS[dir][0]
            yy += DIRS[dir][1]
        else:
...
def flood_fill_perimeter(x, y, region, areaFound, sides):
    if outOfBounds(x, y):
        return 0
    if grid[x][y] != region:
        return 0
    if visited[x][y] & FLOOD_FILL_MASK:
        return 0
    
    visited[x][y] |= FLOOD_FILL_MASK
    perimeter[x][y] = sides
    area[x][y] = areaFound

    flood_fill_perimeter(x-1, y, region, areaFound, sides)
    flood_fill_perimeter(x+1, y, region, areaFound, sides)
    flood_fill_perimeter(x, y-1, region, areaFound, sides)
    flood_fill_perimeter(x, y+1, region, areaFound, sides)
...
for x in range(1, len(grid)-1):
    for y in range(1, len(grid[0])-1):
        ...
        if not (visited[x][y] & PERIMETER_OUT_MASKS[1]) and grid[x][y-1] != grid[x][y] and grid[x-1][y] != grid[x][y]:
            sides = calculate_outside_perimeter(x, y, grid[x][y])
            flood_fill_perimeter(x, y, grid[x][y], areaFound, sides)
